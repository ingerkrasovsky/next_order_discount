...
- **гибкие условия** срабатывания: триггерные статусы заказа, диапазон суммы заказа-источника, окно активности по датам, номер заказа покупателя (например, «только первый заказ»), исключение гостевых заказов, а также списочные условия в режиме **All / Include / Exclude** — группы клиентов, страны, валюты, категории товаров и бренды;
...
Состояние фоновой очереди теперь находится только во вкладке [Cron/Tools](#t20), в блоке **Dispatch queue**.
...
При автоматической отправке модуль берёт текст строго для выбранного языка письма и не подмешивает сохранённый текст другого языка. Если тема или HTML для этого языка не сохранены, используется встроенный шаблон. В комплекте есть локализованные шаблоны **EN** и **FR**; для остальных языков встроенный запасной вариант — **EN**, поэтому перед включением правила проверьте и сохраните текст на всех нужных языках магазина.
...
3. Создайте правило во вкладке **Rules → Add a rule**:
   ...
   - **Conditions**: триггерные статусы и, при необходимости, ограничения (группы, страны, суммы, номер заказа и т.д.); для правил первого/повторного заказа решите, нужно ли включить **Registered customers only**;
...
5. Проверьте на тестовом заказе: переведите заказ в триггерный статус и убедитесь, что во вкладке **Coupons** появился купон, а письмо было отправлено. Если первая попытка не удалась, запустите фоновые задачи вручную во вкладке **Cron/Tools**.
