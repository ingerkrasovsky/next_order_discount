...
Работа модуля настраивается с помощью **правил**. В каждом правиле вы указываете, при каких условиях покупатель получит купон и какая скидка будет действовать. Таких правил можно создать столько, сколько нужно. Когда заказ соответствует условиям одного из них, модуль создаёт купон и сам управляет всем его жизненным циклом: отправляет письмо покупателю и напоминания, следит за сроком действия и отменяет купон при возврате заказа.
...
- **правила скидок**: создавайте столько правил, сколько нужно, задавайте для каждого свои условия и скидку и управляйте порядком их применения с помощью приоритета;
...
- **гибкие условия** срабатывания: статусы заказа, при которых выдаётся купон, диапазон суммы заказа-источника, окно активности по датам, номер заказа покупателя (например, «только первый заказ»), исключение гостевых заказов, а также списочные условия в режиме **All / Include / Exclude** — группы клиентов, страны, валюты, категории товаров и бренды;
...
- **свои письма на каждое правило**: купонное письмо и два письма-напоминания, отдельно **на каждый язык магазина**, с предпросмотром, отправкой тестового письма и выбором языка при ручной отправке из списка купонов;
...
- фоновые задачи по **cron** (очередь писем, планирование напоминаний, истечение купонов) с помощником настройки, проверкой состояния и ручным запуском;
- **Dashboard** — воронка купонов (сгенерировано → отправлено → напомнено → использовано → просрочено → отменено), конверсия и график ежедневной динамики за последние 30 дней;
...
4. [Вкладка Dashboard: воронка и динамика](#t4)
...
7. [Вкладка Coupons: выданные купоны](#t15)
    - [Фильтры](#t16)
    - [Столбцы таблицы](#t17)
    - [Ручная отправка письма и напоминаний](#t18)
8. [Вкладка Settings](#t19)
9. [Вкладка Cron/Tools: фоновые задачи](#t20)
    - [Настройка cron](#t21)
    - [Состояние фоновых задач](#t22)
...
- **Dashboard** — воронка купонов, конверсия и ежедневная динамика выдачи, отправки и использования купонов за последние 30 дней. Открывается по умолчанию.
...
- **Cron/Tools** — настройка фоновых задач (cron): помощник установки, ссылки на задачи, проверка состояния, ручной запуск, снимок очереди.
...
Если включён мультимагазин, всё читается и сохраняется в **контексте выбранного вверху магазина**. В контексте «All shops» списки купонов, воронка, график динамики и очередь показываются суммарно по всем магазинам.
...
4. **Письмо.** Сразу после создания купона модуль пытается отправить письмо покупателю. Если отправка не удалась, письмо ставится в очередь, а cron повторяет попытку (см. [Cron/Tools](#t20)). Автоматическая отправка использует язык заказа-источника; если он недоступен, модуль последовательно использует язык аккаунта покупателя и язык магазина по умолчанию.
...
## 4. Вкладка Dashboard: воронка и динамика

Вкладка **Dashboard** показывает общие результаты работы правил и изменение ключевых показателей по дням в контексте выбранного магазина.

### Воронка (Coupon funnel)
Шесть плиток с числом купонов на каждом этапе и долей от сгенерированных. Процент показан рядом с числом и на цветной полосе под ним; для **Generated** процент не выводится, потому что это базовое значение:
...
> Этапы воронки могут пересекаться и не обязаны складываться в 100%: например, использованный или просроченный купон по-прежнему учитывается среди ранее отправленных, если купонное письмо успешно ушло.

### Daily dynamics (ежедневная динамика)

Линейный график показывает показатели по каждому дню за последние **30 дней**, включая сегодня:

- **Generated** — сколько купонов создано в этот день;
- **Emailed** — сколько купонных писем успешно отправлено в этот день;
- **Used** — сколько купонов использовано в этот день.

Все три линии используют одну шкалу, поэтому их можно сравнивать напрямую. Наведите курсор на точку, чтобы увидеть значения за день. Нажатие на показатель в легенде скрывает или возвращает соответствующую линию; после этого масштаб графика автоматически пересчитывается. Если за последние 30 дней нет ни одного события, график не показывается.

> В мультимагазине данные показываются в контексте выбранного вверху магазина (в «All shops» — суммарно).

_Скриншот: Вкладка Dashboard (воронка и ежедневная динамика)._
![img_3.png](img_3.png)
...
Это основная рабочая область: здесь перечислены все правила выдачи купонов для текущего магазина. Кнопка **Add a rule** (в шапке страницы) открывает форму создания правила. После новой установки модуль не создаёт активное правило автоматически: пока вы сами не добавите и не включите правило, купоны выдаваться не будут.

Если правил ещё нет, вместо таблицы показывается предупреждение **No discount rules yet** и дополнительная кнопка **Add a rule** — нажмите её или одноимённую кнопку в шапке страницы, чтобы создать первое правило.
...
- **Discount value** — величина скидки. Для процента ограничена 100. Подпись справа (`%` или знак валюты) подставляется автоматически по выбранному типу; при типе **Free shipping** поле скрывается (величина скидки не нужна).
...
**Trigger on order statuses** — статусы, в которых заказ может выдать купон. Правило проверяется при создании заказа и при каждом изменении его статуса. Пустой список означает, что статус не ограничивает правило. Для выбора нескольких статусов удерживайте Ctrl/Cmd. Купон выдаётся только если заказ соответствует и остальным условиям правила.
...
Сам список для выбора появляется только при режимах **Only the selected** / **All except the selected**; при режиме **All** он скрыт, чтобы не мешать.
...
- **Customer order number** — сколько заказов должно быть у покупателя (**Min** / **Max**). Оба значения `1` = только первый заказ. `0` = без ограничения. Заказы одного покупателя считаются в текущем магазине по e-mail, даже если PrestaShop создал для того же адреса несколько записей клиента; текущий заказ уже входит в это число.
- **Registered customers only** — если **Yes**, гостевые заказы не участвуют в правиле. Если **No**, правило одинаково проверяет зарегистрированных покупателей и гостей. Рекомендуется включать для сценариев «первый заказ» и возврата клиента: при гостевом оформлении PrestaShop может каждый раз создавать новую запись покупателя, поэтому надёжно распознать вернувшегося гостя удаётся только при совпадении e-mail.
...
_Скриншот: Вкладка Conditions._
![img_8.png](img_8.png)
...
- под полем HTML — строка **Available placeholders (click to insert)**: кликабельные «чипы» плейсхолдеров. Клик по чипу вставляет плейсхолдер прямо в поле HTML на позицию курсора — удобно не набирать их вручную.

Плейсхолдеры, которые подставляются реальными значениями при отправке:

- купон: `{coupon_code}`, `{coupon_value}`, `{valid_to}`, `{minimum_amount}`;
- покупатель: `{customer_firstname}`, `{customer_lastname}`, `{customer_fullname}`, `{customer_title}` (обращение, например «Mr»/«Mrs»; пусто, если пол не задан), `{customer_email}`;
- магазин: `{shop_name}`, `{shop_url}`, `{shop_logo}`. Плейсхолдер `{shop_url}` поддерживается при отправке, но при необходимости его следует ввести в HTML вручную.

Для каждого языка письма используются свои тема и HTML — тексты из других языковых версий не подставляются. Если для выбранного языка тема или HTML не сохранены, модуль использует встроенный шаблон: французский для **FR**, английский — для **EN** и всех остальных языков. Перед включением правила заполните и сохраните письма для всех языков магазина.
...
- **Valid until** — срок действия купона; дата и время отображаются в формате текущей локали PrestaShop.
- **Generated** — дата и время создания в формате текущей локали PrestaShop.
...
### Ручная отправка письма и напоминаний

Пока купон ещё **можно использовать** (не used / expired / canceled), в столбце Actions доступны:

- **Language of the email to send** — выбор языка ручной отправки (показывается, если в магазине установлено больше одного языка). По умолчанию выбран язык заказа-источника; выбранное значение применяется и к повторной отправке купона, и к ручной отправке напоминания.
- кнопка с **конвертом** (**Resend** в подсказке) — повторно отправить покупателю купонное письмо;
- кнопки с **колокольчиком и номером 1 / 2** — отправить соответствующее напоминание немедленно (появляются, если у правила купона включены эти напоминания).

Для использованных, просроченных и отменённых купонов действия недоступны — их письмо и напоминания уже не имеют смысла.

_Скриншот: Кнопки повторной отправки и напоминаний в строке купона._
![img_13.png](img_13.png)
...
После создания купона модуль сразу пытается отправить основное письмо. Если отправка не удалась, письмо попадает в очередь, а **cron** повторяет попытку. Cron также планирует и отправляет письма-напоминания и переводит купоны с истёкшим сроком в **expired**.
...
- **One-click install** — если на сервере доступна автоматическая настройка crontab, кнопка **Install cron automatically** добавит нужную строку, а **Remove cron** удалит её. Строка помечается маркерами и также удаляется при удалении модуля. Если автоустановка недоступна (например, `shell_exec` заблокирован на shared-хостинге), модуль объяснит причину и предложит скопировать строку вручную.
...
_Скриншот: Блок настройки cron._
...
- **Process the dispatch queue** — повторяет неудавшуюся отправку основного письма и отправляет запланированные напоминания. Рекомендуется **каждые 5 минут**.
- **Plan coupon reminders** — находит напоминания, срок которых наступил, и ставит их в очередь. Рекомендуется **каждые 30 минут**.
- **Expire lapsed coupons** — переводит просроченные купоны в expired. Рекомендуется **раз в день**.

Столбец **Last run** показывает состояние задачи: **OK** (запускается вовремя), **Late** (запаздывает), **Not running** (давно не запускалась), **Never run** (ни разу). Столбец **Lock** показывает, выполняется ли задача прямо сейчас (**Running**) или свободна (**Free**) — блокировка не даёт двум запускам пересечься.

Отдельный блок **Managed cron** сообщает, установлена ли cron-строка самим модулем.

_Скриншот: Таблица задач с расписанием и текущим состоянием._
...
Внизу расположен снимок очереди: **Pending / Processing / Done / Failed**. Растущий Pending или заметный Failed — повод проверить cron и настройки почты.
...
Список постраничный. Глубина логирования зависит от **Debug mode**, а срок хранения — от **Keep logs for** (обе настройки — во вкладке [Settings](#t19)). В контексте конкретного магазина также показываются общие записи cron и очереди, созданные без привязки к одному магазину: это позволяет видеть фоновые ошибки даже при выключенном режиме отладки. Запись о выпуске купона содержит `id_lang` и ISO-код языка, на котором будет отправлено автоматическое письмо.
...
3. Создайте правило во вкладке **Rules → Add a rule**:
   ...
   - **Conditions**: статусы заказа, при которых выдаётся купон, и при необходимости ограничения (группы, страны, суммы, номер заказа и т. д.); для правил первого/повторного заказа решите, нужно ли включить **Registered customers only**;
...
5. Проверьте правило на тестовом заказе: переведите заказ в один из указанных в правиле статусов и убедитесь, что во вкладке **Coupons** появился купон, а письмо было отправлено. Если первая попытка не удалась, запустите фоновые задачи вручную во вкладке **Cron/Tools**.
6. Следите за результатами во вкладке **Dashboard**: оценивайте воронку, конверсию и ежедневную динамику за последние 30 дней.
...
4. Заказ подходит под **все** условия правила: диапазон суммы, окно дат, номер заказа покупателя, тип покупателя (гость или зарегистрированный), группы/страны/валюты/категории/бренды.
...
1. Проверьте почтовую конфигурацию магазина; отправьте **Send test email** из вкладки Email правила.
2. Для конкретного купона можно выбрать нужный язык и нажать кнопку с конвертом во вкладке Coupons.
3. Если первая попытка отправки завершилась ошибкой, убедитесь, что работает **cron** (Cron/Tools → задача **Process the dispatch queue**, статус **OK**), и нажмите **Run all tasks now** для повторной попытки.
4. В очереди не должно быть застрявших **Pending** / растущих **Failed** (Cron/Tools → **Dispatch queue**).
5. Если письмо пришло не на том языке, проверьте текст этого языка во вкладке **Email** правила. Автоматически модуль использует язык заказа; при ручной отправке — язык, выбранный рядом с кнопками действий.
