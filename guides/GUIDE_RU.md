...
4. **Письмо.** Купонное письмо ставится в очередь отправки и уходит покупателю при ближайшем проходе cron (см. [Cron/Tools](#t20)). Автоматическая отправка использует язык заказа-источника; если он недоступен, модуль последовательно использует язык аккаунта покупателя и язык магазина по умолчанию.
...
2. В очереди нет застрявших **Pending** / растущих **Failed** (Cron/Tools → **Dispatch queue**).
3. Проверьте почтовую конфигурацию магазина; отправьте **Send test email** из вкладки Email правила.
4. Для конкретного купона можно выбрать нужный язык и нажать кнопку с конвертом во вкладке Coupons.
