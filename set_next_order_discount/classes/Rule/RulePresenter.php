 ...
 * iterates {@see SnodRuleConditionSchema} rather than hard-coding condition
 * types.
 */
class SnodRulePresenter
{
    /**
    ...
    private function discountLabel(array $rule, $currencySign, array $labels)
    {
        ...
        $value = SnodRuleValueFormatter::decimal($rule['discount_value']);

        if ($type === SnodRuleRepository::DISCOUNT_AMOUNT) {
    ...
    {
        $list = [];
        $statusIds = isset($conditions[SnodRuleConditionSchema::TYPE_STATUS])
    ...
    private function summary(array $rule, array $conditions, $currencySign, array $labels)
    {
        ...
        if ($min > 0 && $max > 0) {
            $parts[] = $totalLabel . ' ' . SnodRuleValueFormatter::decimal($min) . '–' . SnodRuleValueFormatter::decimal($max) . ' ' . $currencySign;
        } elseif ($min > 0) {
            $parts[] = $totalLabel . ' ≥ ' . SnodRuleValueFormatter::decimal($min) . ' ' . $currencySign;
        } elseif ($max > 0) {
            $parts[] = $totalLabel . ' ≤ ' . SnodRuleValueFormatter::decimal($max) . ' ' . $currencySign;
        }

        $conditionLabels = isset($labels['conditions']) ? $labels['conditions'] : [];
        foreach (SnodRuleConditionSchema::modeTypes() as $type) {
            $mode = (string) $rule[SnodRuleConditionSchema::modeColumn($type)];
            $count = count((array) (isset($conditions[$type]) ? $conditions[$type] : []));
            if ($mode !== SnodRuleConditionSchema::MODE_ALL && $count > 0) {
                $label = isset($conditionLabels[$type]) ? $conditionLabels[$type] : SnodRuleConditionSchema::label($type);
